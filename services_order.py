...
from models import PedidoCreate, Producto as ProductoModel
import logging
...
class OrderService:
    ...
    @staticmethod
    def crear_pedido(db: Session, pedido_data: PedidoCreate) -> tuple[Pedido, str]:
        ...
        try:
            ...
            for item_data in pedido_data.items:
                producto = db.query(ProductoModel).filter(
                    ProductoModel.id == item_data.producto_id
                ).first()
                
                if not producto:
                    return pedido, f"Producto {item_data.producto_id} no encontrado"
                
                if producto.stock < item_data.cantidad:
                    return pedido, f"Stock insuficiente para {producto.nombre}"
